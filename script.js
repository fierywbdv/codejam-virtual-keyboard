...
document.querySelector('.inform').innerHTML = '<b>1.Переключение языка клавиатуры ctrl+alt;<br> 2.Информация для проверяющих (cross-check): <br> - курсор в поле для ввода (textarea) ставить не нужно, проверяем фактическим нажатием на кнопки и мышью (символы в textarea выводятся автоматически);  <br> - ошибок eslint в консоле у меня нет;<br>-реализованы все предусмотренные техническим заданием функции клавиатуры, в том числе необязательная функция переключения алфавита Caps-lock (включается только физическим нажатием кнопки, а не мыши - это не ошибка, т.к. реализация кнопки не является обязательной); <br> - не реализована кнопка del (реализация не обязательна, в тз есть фраза "....del(если она присутствует)" ); <br>-не реализованы свойства кнопки shift (не обязательно согласно ТЗ). <br> PS. Если у Вас появились замечания, оставляйте пожалуйста свои данные для обратной связи (ник в Diskord в идеале)!<b>';
